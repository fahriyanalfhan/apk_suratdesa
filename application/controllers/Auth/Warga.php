		...
		if ($this->form_validation->run() == FALSE) {
			...
			if (!$ceknik) {
				...
				$config = array(
					...
					'smtp_port' => 465,
					'smtp_user' => 'coffincoffexyz@gmail.com',
					'smtp_pass' => 'nmnzngrooeylcvug',
					'mailtype'  => 'html',
					'charset'   => 'iso-8859-1',
					'wordwrap'  => TRUE
				);
				...
				$this->email->from('coffincoffexyz@gmail.com', 'Desa Lagadar');
				$this->email->to($email);
				$this->email->subject('Pendaftaran Akun Desa Lagadar Berhasil');
				$this->email->message('Terima kasih telah mendaftar, Akun Desa Lagadar Anda telah berhasil dibuat, Silahkan Login untuk
				melakukan Pengajuan Surat! ');

				if ($this->email->send()) {
					$this->session->set_flashdata('success', 'Data berhasil disimpan dan email notifikasi telah dikirim!');
				} else {
					$this->session->set_flashdata('success', 'Data berhasil disimpan, tetapi email notifikasi gagal dikirim.');
				}
