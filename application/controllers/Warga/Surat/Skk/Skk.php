...
class Skk extends CI_Controller
{
	...
	public function create()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($cek) {
				...
				$d2 = trim($date);
				//acak nama gambar
				$extensi1 = explode('.', $file_kem);
				$extensi = strtolower(end($extensi1));
				$acak_angka =  rand(1, 999);
				$filekem = str_replace('', '', 'skk-id-' . $this->session->userdata('id_warga') . '-tgl' . $d2 . '-' . $acak_angka . '.' . $extensi);
				...
				$config['upload_path'] = './assets/file_kem/'; //folder penyimpanana gambar
				$config['file_name'] = $filekem;
				$config['allowed_types'] = 'jpeg|png|jpg|JPEG|PNG|JPG';
				$config['max_size'] = '3024';
				$config['remove_space'] = TRUE;
				$this->load->library('upload', $config);
				$this->upload->initialize($config);
				if (!$this->upload->do_upload('file_kem')) {
					$this->session->set_flashdata('danger', $this->upload->display_errors());
	...
	public function update()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_surat->skk_update($data, $id);
			$this->session->set_flashdata('success', 'Permohonan surat berhasil dibuat !');
			redirect('history-surat-kematian', 'refresh');
		}
	}
}
