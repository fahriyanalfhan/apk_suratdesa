...
class Sktm extends CI_Controller
{
	...
	public function create()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($cek) {
				...
				$extensi = strtolower(end($extensi1));
				$acak_angka =  rand(1, 999);
				$filert = str_replace('', '', 'sktm-id-' . $this->session->userdata('id_warga') . '-tgl' . $d2 . '-' . $acak_angka . '.' . $extensi);


				if ($file_kk == '' || $file_ktp == '' || $file_rumah == '' || $file_rt == '') {
	...
	public function update()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_surat->sktm_update($data, $id);
			$this->session->set_flashdata('success', 'Permohonan surat berhasil diupdate !');
			redirect('history-surat-tidak-mampu', 'refresh');
		}
	}
}
